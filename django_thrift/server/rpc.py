...
from django_thrift.publicMethod import SocketInfo
from django_thrift.kazooAPI import ForServer
...
def create_processor():
    ...
    for app in [x for x in settings.INSTALLED_APPS if not x.startswith("django")]:
        importlib.import_module("%s.service" % app)
...
def make_server(host="localhost", port=9090, unix_socket=None,
    ...
    host = SocketInfo().getCurrentHost()
    port = SocketInfo().getAvailablePort()
    if unix_socket:
